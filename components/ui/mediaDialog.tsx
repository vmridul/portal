...
export const MediaDialog = ({
    ...
    const mediaFiles = useQuery(api.messages.getMedia, { room_id, type });
            ...
            if (selectedIndex !== null && activeData) {
                ...
                if (e.key === "ArrowRight") {
                    setSelectedIndex((prev) =>
                        prev !== null && prev < activeData.length - 1 ? prev + 1 : prev
                    );
                }
            }
        };
    ...
    return (
        <>
            {selectedIndex !== null && (
                <div
                    className="fixed inset-0 z-[10000] bg-black/95 transition-opacity flex items-center justify-center p-4 md:p-10"
                    onClick={() => setSelectedIndex(null)}
                >
                    {activeData[selectedIndex].type?.startsWith("video/") ? (
                        <video
                            src={activeData[selectedIndex].file_url!}
                            controls
                            autoPlay
                    ...
                    <button
                        ...
                        className="absolute top-6 right-6 text-white/60 hover:text-white/80 bg-black/50 rounded-full p-2"
                    >
                        <X className="w-6 h-6" />
                    </button>
                    ...
                    {selectedIndex > 0 && (
                        <button
                            ...
                            className="absolute left-6 hover:bg-theme-hover text-white p-2 rounded-[8px] transition-colors"
                        >
                    ...
                    {selectedIndex < activeData.length - 1 && (
                        <button
                            ...
                            className="absolute right-6 hover:bg-theme-hover text-white p-2 rounded-[8px] transition-colors"
                        >
            ...
            >
                ...
                >
                    <div className="flex flex-col md:flex-row justify-between w-full items-start md:items-center mb-6 gap-4">
                        <div className="flex space-x-2 bg-[#080f17] p-1 rounded-xl">
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-[8px] text-sm transition-all duration-200 ease-out ${activeTab === "images" ? "shadow-md shadow-black/20" : "text-white/50 hover:text-white/80"}`}
                            >
                                Images
                            </button>
                            <button
                                onClick={() => setActiveTab("videos")}
                                style={activeTab === "videos" ? { backgroundColor: color, color: textColor } : {}}
                                className={`px-4 py-1.5 rounded-[8px] text-sm transition-all duration-200 ease-out ${activeTab === "videos" ? "shadow-md shadow-black/20" : "text-white/50 hover:text-white/80"}`}
                            >
                                Videos
                            </button>
                            <button
                                onClick={() => setActiveTab("files")}
                                style={activeTab === "files" ? { backgroundColor: color, color: textColor } : {}}
                                className={`px-4 py-1.5 rounded-[8px] text-sm transition-all duration-200 ease-out ${activeTab === "files" ? "shadow-md shadow-black/20" : "text-white/50 hover:text-white/80"}`}
                            >
                                Files
                            </button>
                        </div>
                        <button onClick={() => setMediaDialog(false)}>
                    ...
                    <div className={`w-full flex-1 overflow-y-auto ${activeTab === "files" ? "flex flex-col gap-2" : "grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 auto-rows-[120px]"}`}>
                        {activeData.length === 0 ? (
                            <div className="col-span-full w-full flex h-[200px] items-center justify-center text-white/40">
                                ...
                                activeTab === "files" ? (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col overflow-hidden">
                                            <span className="text-sm text-white/90 font-medium truncate">{media.file_name || "Document"}</span>
                                    ...
                                    >
                                        ...
                                        {media.type?.startsWith("video/") && (
                                            <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                                                <div className="w-8 h-8 rounded-full bg-black/50 flex flex-col justify-center items-center backdrop-blur-sm">
